...
PEN_UP_WEIGHT = 5.0  # upweight pen-up events to correct ~90% pen-down class imbalance
...
def gaussian_mixture_loss(y, y_lengths, pis, sigmas, rhos, mus, es, eps=1e-8, sigma_eps=1e-4):
    ...
    Pen-up events are weighted by PEN_UP_WEIGHT to correct class imbalance.
    """
    batch_size = y.size(0)
    seq_length = y.size(1)
    ...
    # bivariate Gaussian probability
    ...
    # weight pen-up log-likelihood to compensate for class imbalance
    pen_weight = torch.where(
        y_3.squeeze(2) > 0.5,
        torch.full_like(bernoulli_likelihood, PEN_UP_WEIGHT),
        torch.ones_like(bernoulli_likelihood)
    )

    nll = -(torch.log(gmm_likelihood) + pen_weight * torch.log(bernoulli_likelihood.clamp(min=eps)))
